 ...
 *
 * Builds Sui deposit transaction inputs for a user's connected Sui wallet.
...
import { buildDepositTransactionPayload } from "@/lib/server/sui-execution";
  ...
  try {
    ...
    const result = await prepareSuiDepositTransaction(
      { id, walletAddress: parsed.data.walletAddress },
      {
        ...
        buildTransactionPayload: buildDepositTransactionPayload,
      }
    );
